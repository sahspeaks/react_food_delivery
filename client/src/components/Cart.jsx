...
const Cart = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-40 gap-70 relative">
        {cart && cart?.length > 0 ? (
          <>
            <div className="flex flex-col -my-24 w-full items-start justify-start gap-4 h-[65%] overflow-y-scroll scrollbar-none px-4">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
            ...
            <div className="bg-zinc-800 rounded-t-[0px] w-full h-[40%] flex flex-col items-center justify-center my-40 mb-10 px-4 py-2 gap-10">
              ...
              <motion.button
                {...buttonClick}
                className="bg-orange-400 -my-4  w-[70%] px-4 py-3 text-xl text-headingColor font-semibold hover:bg-orange-500 drop-shadow-md rounded-2xl"
...
export const CartItemCard = ({ index, data }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full flex items-center justify-start bg-zinc-800 rounded-md drop-shadow-md px-6 gap-4"
    >
      <img
        src={data?.imageURL}
