...
import { Avatar, HLogo } from "../assets";
  ...
  return (
    <header className="fixed backdrop-blur-md z-50 inset-x-0 top-0 flex items-center justify-between px-12 md:px-20 py-6 ">
      <NavLink to={"/"} className="flex items-center justify-center gap-4">
        <img src={HLogo} className="w-14 " alt="" />
        <p className="font-semibold text-xl">Homemade Haven</p>
      </NavLink>
        ...
        {user ? (
          ...
          <>
            <NavLink to={"/login"}>
              <motion.div
                {...buttonClick}
                className="px-4 py-2 rounded-md bg-cardOverlay border border-red-300 cursor-pointer"
              >
                Login
              </motion.div>
            </NavLink>
